...
def parse_zone_definitions(raw: list[dict[str, str]] | None) -> list[Zone]:
    """Parse zone definitions from configuration."""
    if not raw:
        return []
...
def _print_zone_summary(log, summary: dict[str, object], label: str) -> None:
    ...
    if not zones or not isinstance(zones, list) or total == 0:
        log(f"No {label} data for zone calculation.")
        return

    # Safe conversion of total to float
    try:
        total_seconds = float(total) if isinstance(total, (int, float)) else 0.0
    ...
    log(f"Total time in calculation: {seconds_to_hms(total_seconds)}")
    ...
    for zone in zones:
        ...
        zone_seconds = zone.get("seconds", 0)
        zone_percent = zone.get("percent", 0)
        
        range_str = format_range(zone_lower, zone_upper)
        
        try:
            seconds_val = float(zone_seconds) if isinstance(zone_seconds, (int, float)) else 0.0
            percent_val = float(zone_percent) if isinstance(zone_percent, (int, float)) else 0.0
        except (ValueError, TypeError):
            seconds_val = 0.0
            percent_val = 0.0
            
        log(
            f"- {zone_name:<20} {range_str:<10} "
            f"{seconds_to_hms(seconds_val):>8} ({percent_val:5.1f}%)"
        )
